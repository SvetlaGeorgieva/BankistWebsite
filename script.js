...
const slider = function () {
  ...
  // slide is a number from 0 to slides.length
  ...
  const goToSlide = function (slide) {
    // Move the slide elements left or right until the selected one is in view
    slides.forEach(
  ...
  // Event handlers
  // -> Navigating between the slide with buttons
  btnRight.addEventListener("click", nextSlide);
  btnLeft.addEventListener("click", prevSlide);

  // -> Navigating between the slides with keyboard
  document.addEventListener("keydown", function (e) {
    if (e.key === "ArrowLeft") prevSlide();
    if (e.key === "ArrowRight") nextSlide();
  });

  // -> Navigating between the slides by clicking on the dots
